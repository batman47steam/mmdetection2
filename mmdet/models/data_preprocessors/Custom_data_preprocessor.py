...
class CostumImgDataPreprocessor(ImgDataPreprocessor):
    ...
    def forward(self, data: dict, training: bool = False) -> Union[dict, list]:
        ...
        _batch_backgrounds = data['backgrounds']
        # Process data with `pseudo_collate`.
        if is_seq_of(_batch_inputs, torch.Tensor):
            batch_inputs = []
            batch_backgrounds = []
            for _batch_input, _batch_background in zip(_batch_inputs, _batch_backgrounds):
                # channel transform
                if self._channel_conversion:
                    _batch_input = _batch_input[[2, 1, 0], ...]
                    _batch_background = _batch_background[[2,1,0], ...]
                # Convert to float after channel conversion to ensure
                # efficiency
                _batch_input = _batch_input.float()
                _batch_background = _batch_background.float()
                # Normalization.
                if self._enable_normalize:
                    ...
                    _batch_input = (_batch_input - self.mean) / self.std # 都减去相同的mean，可能会有点小问题
                    _batch_background = (_batch_background - self.mean) / self.std
                batch_inputs.append(_batch_input)
                batch_backgrounds.append(_batch_background)
            # Pad and stack Tensor.
            batch_inputs = stack_batch(batch_inputs, self.pad_size_divisor,
                                       self.pad_value)
            batch_backgrounds = stack_batch(batch_backgrounds, self.pad_size_divisor,
                                            self.pad_value)
        # Process data with `default_collate`.
            ...
            if self._channel_conversion:
                ...
                _batch_backgrounds = _batch_backgrounds[:, [2,1,0], ...]
            # Convert to float after channel conversion to ensure
            # efficiency
            _batch_inputs = _batch_inputs.float()
            _batch_backgrounds = _batch_backgrounds.float()
            if self._enable_normalize:
                _batch_inputs = (_batch_inputs - self.mean) / self.std
                _batch_backgrounds = (_batch_backgrounds - self.mean) / self.std
            ...
            batch_backgrounds = F.pad(_batch_backgrounds, (0, pad_w, 0, pad_h),
                                      'constant', self.pad_value)
        else:
        ...
        data['backgrounds'] = batch_backgrounds
...
@MODELS.register_module()
class CostumDetDataPreprocessor(CostumImgDataPreprocessor):
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        inputs, backgrounds, data_samples = data['inputs'], data['backgrounds'], data['data_samples']
        ...
        # 先把inputs和backgrounds都concat在一起，是在batch维度上去concate
        inputs = torch.concat((inputs, backgrounds), dim=0)

        #return {'inputs': inputs, 'backgrounds': backgrounds, 'data_samples': data_samples}
